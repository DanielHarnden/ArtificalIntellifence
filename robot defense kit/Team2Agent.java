 ...
 */
public class Team2Agent extends BaseLearningAgent {
	...
	static {
		...
		potentials = new AgentAction[dirs.length * (AirCurrentGenerator.POWER_SETTINGS + 1)];

		int i = 0;
		for(Direction d: dirs) {
			// creates a new directional action with the power set to full
			// power can range from 1 ... AirCurrentGenerator.POWER_SETTINGS
			for (int level = 0; level <= AirCurrentGenerator.POWER_SETTINGS; level++) {
				potentials[i] = new AgentAction(level, d);
			i++;
			}
		}
		...
		for (AirCurrentGenerator acg : sensors.generators.keySet()) {
			...
			if (lastAction.get(acg) != null ) {
				...
				if (justCaptured) {
					...
					qmap.rewardAction(lastAction.get(acg), 1000.0);
					captureCount.put(acg,sensors.generators.get(acg));
				} else {
					switch(lastAction.get(acg).getPower())
					{
						case 1:
							qmap.rewardAction(lastAction.get(acg), -0.01);
							break;

						case 2:
							qmap.rewardAction(lastAction.get(acg), -0.1);
							break;

						case 3:
							qmap.rewardAction(lastAction.get(acg), -1);
							break;

						case 4:
							qmap.rewardAction(lastAction.get(acg), -10);
							break;
